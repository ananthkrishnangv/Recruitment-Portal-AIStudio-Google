...
const StatCard: React.FC<{ title: string; value: string; icon: React.ReactNode; colorClass: string; trend?: string }> = ({ title, value, icon, colorClass, trend }) => (
  <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 group">
    <div className="flex justify-between items-start mb-6">
      ...
      {trend && (
        <span className="flex items-center text-[10px] font-black text-emerald-500 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100 font-mono">
      ...
      <aside className="w-full lg:w-72 bg-white border-r border-slate-200 p-6 flex flex-col z-40">
        <div className="flex items-center px-4 mb-10">
          ...
          <div className="flex flex-col">
             ...
             <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest font-mono">v4.0.2 Stable</span>
          </div>
        </div>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-12">
           ...
           <div className="flex items-center space-x-3">
              <div className="px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-xl text-emerald-600 text-[10px] font-black uppercase tracking-widest flex items-center font-mono">
                       ...
                       <button key={p.id} onClick={() => alert('Generating package for ' + p.code)} className="w-full p-4 bg-white/10 hover:bg-white/20 rounded-2xl flex items-center justify-between transition-all border border-white/10 group">
                          <div className="flex items-center text-left">
                             ...
                             <div className="flex flex-col">
                                ...
                                <span className="text-[10px] text-white/40 uppercase font-bold tracking-widest font-mono">{p.code}</span>
                             </div>
                          </div>
                  ...
                  <div className="flex flex-col md:flex-row items-center p-2">
                    <div className="p-8 flex-1">
                      <div className="flex items-center space-x-3 mb-4">
                        <span className="px-3 py-1 bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-lg font-mono">
                          {post.code}
                        </span>
                        <span className="text-[10px] font-black text-brand-600 uppercase tracking-widest bg-brand-50 px-2 py-1 rounded-lg border border-brand-100 font-mono">
                          {post.status}
                        </span>
                      </div>
          ...
          <div className="space-y-8 animate-fadeIn">
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="w-full md:w-96">
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Select Drive to Scrutinize</p>
                   <select 
                    className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-900 focus:bg-white transition-all outline-none"
                    value={selectedPostId}
                    onChange={(e) => setSelectedPostId(e.target.value)}
                   >
                     <option value="">Select Target Recruitment Project</option>
                     {posts.map(p => <option key={p.id} value={p.id}>{p.code} - {p.title}</option>)}
                   </select>
                </div>
                <div className="flex items-center gap-3">
                   <button onClick={() => bulkUpdateStatus(selectedAppIds, ApplicationStatus.UNDER_SCRUTINY)} disabled={selectedAppIds.length === 0} className="px-6 py-4 bg-amber-500 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-amber-100 hover:bg-amber-600 transition-all active:scale-95 disabled:opacity-20">Hold in Review</button>
                   <button onClick={() => bulkUpdateStatus(selectedAppIds, ApplicationStatus.REJECTED)} disabled={selectedAppIds.length === 0} className="px-6 py-4 bg-red-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-red-100 hover:bg-red-700 transition-all active:scale-95 disabled:opacity-20">Commit Rejection</button>
                   <div className="w-px h-10 bg-slate-200 mx-2 hidden md:block"></div>
                   <button onClick={() => handlePushToDirector('shortlist')} disabled={!selectedPostId} className="px-8 py-4 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-slate-200 hover:bg-slate-800 transition-all active:scale-95 disabled:opacity-20 flex items-center">
                     Push to Director <Send size={14} className="ml-3 opacity-50"/>
                   </button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <thead>
                      <tr className="bg-slate-50/50 border-b border-slate-100">
                        <th className="px-8 py-5">
                          <input type="checkbox" className="rounded-lg text-brand-600 focus:ring-brand-500 w-5 h-5 border-slate-200" />
                        </th>
                        <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Candidate Profile</th>
                        <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Application ID</th>
                        <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Academics</th>
                        <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Internal Status</th>
                        <th className="px-8 py-5 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {applications.filter(app => !selectedPostId || app.postId === selectedPostId).map((app, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                           <td className="px-8 py-5">
                              <input 
                                type="checkbox" 
                                className="rounded-lg text-brand-600 focus:ring-brand-500 w-5 h-5 border-slate-200" 
                           ...
                           <td className="px-8 py-5">
                              <div className="flex items-center">
                                 <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center font-black text-slate-400 mr-4 shadow-inner">
                                    {app.personalDetails.fullName.charAt(0)}
                                 </div>
                                 <div className="flex flex-col">
                                    <span className="font-black text-slate-900 uppercase text-xs">{app.personalDetails.fullName}</span>
                                    <span className="text-[10px] font-bold text-slate-400">{app.personalDetails.mobile}</span>
                                 </div>
                              </div>
                           </td>
                           <td className="px-8 py-5 font-mono text-[10px] font-black text-slate-600">
                              {app.applicationNumber}
                           </td>
                           <td className="px-8 py-5">
                              <div className="flex items-center text-xs font-bold text-slate-700">
                                 <div className="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center mr-2 text-[10px] font-mono">92%</div>
                                 <span>B.Tech CSE</span>
                              </div>
                           </td>
                           <td className="px-8 py-5">
                              <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border shadow-sm font-mono ${
                                app.status === ApplicationStatus.REJECTED 
                                  ? 'bg-red-50 border-red-100 text-red-600' 
                                  : app.status === ApplicationStatus.UNDER_SCRUTINY ? 'bg-amber-50 border-amber-100 text-amber-600' : 'bg-brand-50 border-brand-100 text-brand-600'
                              }`}>
                                {app.status}
                              </span>
                           </td>
                           <td className="px-8 py-5 text-right">
                              <button className="p-3 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-brand-600 hover:border-brand-200 hover:shadow-lg transition-all active:scale-95">
                                 <Eye size={18}/>
                              </button>
                           </td>
                        </tr>
               ...
               {applications.filter(app => !selectedPostId || app.postId === selectedPostId).length === 0 && (
                 <div className="py-32 text-center flex flex-col items-center">
                    ...
                    <h3 className="text-xl font-heading font-black text-slate-900 mb-2">Registry is Clear</h3>
                    <p className="text-slate-400 font-medium">No pending applications for the selected drive node.</p>
                 </div>
               )}
            </div>
          </div>
                                        ...
                                        <div key={app.applicationNumber} className="flex items-center justify-between py-2 border-b border-slate-200 last:border-0">
                                            <div className="flex items-center">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-slate-500 font-mono">{app.applicationNumber}</p>
                                                </div>
                                            </div>
                                            <span className="text-[10px] bg-white px-2 py-1 rounded border font-mono">{app.status}</span>
                                        </div>
                                    ))}
                                    {applications.filter(a => !selectedPostId || a.postId === selectedPostId).length === 0 && <p className="text-xs text-slate-400 text-center py-4">No candidates found.</p>}
                               </div>
                           ...
                           <div>
                               ...
                               <div className="flex gap-4">
                                   {['email', 'whatsapp', 'telegram'].map(channel => (
                                       <button 
                                            key={channel}
                                            onClick={() => setOutreachChannels({...outreachChannels, [channel as keyof typeof outreachChannels]: !outreachChannels[channel as keyof typeof outreachChannels]})}
               ...
               <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
                   <div className="p-8 border-b border-slate-100 flex justify-between items-center">
                       ...
                       <div className="flex gap-2">
                           <span className="px-3 py-1 bg-red-50 text-red-600 rounded-lg text-xs font-bold border border-red-100 font-mono">{tickets.filter(t => t.status === 'OPEN').length} Open</span>
                           <span className="px-3 py-1 bg-green-50 text-green-600 rounded-lg text-xs font-bold border border-green-100 font-mono">{tickets.filter(t => t.status === 'RESOLVED').length} Resolved</span>
                       </div>
                   </div>
                       ...
                       <tbody className="divide-y divide-slate-100">
                           {tickets.map(ticket => (
                               <tr key={ticket.id} className="hover:bg-slate-50/50 group relative">
                                   ...
                                   <td className="px-8 py-4"><span className={`text-[10px] font-black uppercase px-2 py-1 rounded font-mono ${ticket.status === 'OPEN' ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>{ticket.status}</span></td>
